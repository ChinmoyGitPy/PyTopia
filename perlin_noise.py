...
def generate_perlin_noise_2d(width, height, cell_size, seed=None):
    ...
    for row in range(height):
        ...
        d00 = np.dstack((lx_row, np.full_like(lx_row, ly)))[0]
        d10 = np.dstack((lx_row - 1, np.full_like(lx_row, ly)))[0]
        d01 = np.dstack((lx_row, np.full_like(lx_row, ly - 1)))[0]
...
def generate_fractal_noise_2d(width, height, cell_size, octaves=4, persistence=0.5, lacunarity=2.0, seed=None):
    ...
    for octave in range(octaves):
        ...
        # IMPORTANT: this must call generate_perlin_noise_2d (single-octave
        # noise), NOT generate_fractal_noise_2d, or it recurses into itself
        # forever and crashes with RecursionError.
        layer = generate_perlin_noise_2d(width, height, max(2, int(frequency_cell_size)), seed=octave_seed)
        total += layer * amplitude
        max_amplitude += amplitude
        amplitude *= persistence
        frequency_cell_size = max(2, frequency_cell_size / lacunarity)
...
def sample_grid(rows, cols, cell_size=4, octaves=3, persistence=0.5, lacunarity=2.0, seed=None):
    noise_map = generate_fractal_noise_2d(
        width=cols, height=rows, cell_size=cell_size,
